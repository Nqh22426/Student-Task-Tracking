...
// Advanced query to get tasks with grades info
$stmt = $pdo->prepare("
    SELECT t.*, 
           t.grades_sent,
           CASE 
           ...
           (SELECT s.grade FROM submissions s WHERE s.task_id = t.id AND s.student_id = ? ORDER BY s.submission_date DESC LIMIT 1) as student_grade
    FROM tasks t
    WHERE t.class_id = ? 
...
");
$stmt->execute([$student_id, $student_id, $class_id, $month_start, $month_end, $month_start, $month_end, $month_start, $month_end]);
$tasks = $stmt->fetchAll();
...
foreach ($tasks as $task) {
    ...
    if ($start_month == $month && $start_year == $year) {
        ...
        $task_by_day[$start_day][] = [
            ...
            'student_grade' => $task['student_grade']
        ];
    }
...
<body>
                                ...
                                <div class="task-container">
                                    <?php if (isset($task_by_day[$day])): ?>
                                        <?php foreach ($task_by_day[$day] as $task_item): ?>
                                            ...
                                            // Get grade display
                                            $grade_display = '';
                                            if ($task_item['grades_sent']) {
                                                if ($task_item['has_submission']) {
                                                    $grade_display = "<span class='grade-badge'>(Grade: {$task_item['student_grade']})</span>";
                                                } else {
                                                    $grade_display = "<span class='grade-badge'>(Grade: 0 <span class='text-danger'>- Not submitted</span>)</span>";
                                                }
                                            }
                                            ?>
                                            <div class="task-item task-<?php echo $task_item['type']; ?>" 
                                                onclick="openTaskDetails(<?php echo $task_item['id']; ?>)">
                                                <?php echo htmlspecialchars($task_item['title']) . $grade_display; ?>
                                            </div>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </div>
    ...
    <script src="task_ajax.js"></script>
    <script src="calendar_tasks.js"></script>
    
    <script src="submission_fix.js"></script>
</body>
</html>
