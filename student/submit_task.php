...
// Tạo bảng submissions nếu chưa tồn tại (không cần transaction)
try {
    $stmt = $pdo->prepare("
        CREATE TABLE IF NOT EXISTS submissions (
            ...
            file_path VARCHAR(255) DEFAULT NULL, /* Giữ lại cột nhưng cho phép NULL */
...
try {
    ...
    // Cập nhật hoặc thêm mới submission
    if ($existing) {
...
// Cập nhật bảng student_progress nếu tồn tại (không cần transaction)
try {
    ...
    // Lỗi ở bảng student_progress không cần xóa submission đã lưu thành công
    error_log("Error updating student_progress: " . $e->getMessage());
    // Nhưng vẫn thông báo thành công vì submission đã được lưu
}
...
if (isset($_POST['ajax'])) {
    ...
    // Validate the referer for security
