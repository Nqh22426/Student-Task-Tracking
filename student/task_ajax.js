...
function openTaskDetails(taskId) {
    ...
    fetch(`get_task_details.php?task_id=${taskId}`)
        ...
        .then(data => {
            ...
            console.log("Task status:", statusText); // Debug log
            ...
            // Add grade information if grades have been sent
            if (data.grades_sent) {
                let gradeDisplay = `<div class="mb-3 mt-3">
                    <h6 class="description-label">Grade:</h6>
                    <span class="grade-display">${data.grade !== undefined ? data.grade : '0'}</span>`;
                
                // Add 'Not submitted' note if applicable
                if (data.no_submission) {
                    gradeDisplay += ` <span class="text-danger">(Not submitted)</span>`;
                }
                
                gradeDisplay += `</div>`;
                taskDetailsHTML += gradeDisplay;
            }
            ...
            if (data.has_submission) {
                console.log("Task has submission:", data.submission_filename); // Debug log
                console.log("Task is locked:", data.is_locked ? "Yes" : "No"); // Debug log is_locked status
                
                // Check for is_locked property
                console.log("Task lock status:", data.is_locked);
                
                taskDetailsHTML += `                    <div class="submission-container" style="border: none; background-color: transparent; margin-bottom: 0;">                        <div class="submission-content" style="padding: 0; position: relative; padding-bottom: 45px;">                            <a href="${data.submission_path}" target="_blank" class="file-btn">                                <i class="bi bi-file-pdf me-2"></i>${data.submission_filename}                            </a>                            ${data.is_locked ? '' : `                                <a href="unsubmit_task.php?task_id=${data.id}&redirect=class_details.php?id=${data.class_id}"                                    class="btn btn-outline-danger unsubmit-btn"                                   onclick="return confirm('Are you sure you want to remove this submission? You can submit again later.')">                                    <i class="bi bi-x-circle me-1"></i> Unsubmit                                </a>                            `}                        </div>                    </div>                `;
            } else if (statusText === 'Upcoming') {
                console.log("Task is upcoming, showing info message"); // Debug log
                ...
                console.log("Task is locked, showing lock message"); // Debug log
                ...
                console.log("Task is " + statusText + ", showing upload form"); // Debug log
            ...
            // AGGRESSIVE HANDLING: Find and hide any unsubmit buttons if task is locked
            if (data.is_locked) {
                setTimeout(function() {
                    document.querySelectorAll('a[href*="unsubmit_task.php"], .unsubmit-btn').forEach(function(btn) {
                        ...
                        btn.style.opacity = '0';
                        console.log("DIRECT APPROACH: Hiding unsubmit button");
                    });
                    
                    // Also hide upload forms if task is locked
                    hideUploadForms();
                }, 0);
            }
            ...
            if (!data.has_submission && (statusText === 'Ongoing' || statusText === 'Completed') && !data.is_locked) {
                console.log("Initializing file upload"); // Debug log
...
document.addEventListener('DOMContentLoaded', function() {
    // This will work if the lock/unlock happens while the modal is open
    // (e.g., if the teacher and student are looking at the same task)
...
// Initialize file upload functionality - make it globally accessible
window.initializeFileUpload = function() {
    ...
    if (uploadArea && fileInput) {
        console.log("File upload components found and initialized"); // Debug log
        ...
        if (removeFileBtn) {
            removeFileBtn.addEventListener('click', function(e) {
                ...
                console.log("File selection removed"); // Debug log
            });
        }
    } else {
        console.warn("Could not find file upload components"); // Debug log
    }
}

// Function to directly hide unsubmit buttons
function forceHideUnsubmitButtons() {
    console.log("Force hiding unsubmit buttons");
...
function checkAndUpdateUnsubmitVisibility() {
    ...
    console.log("Checking unsubmit visibility, isLocked:", isLocked);
...
function hideUploadForms() {
    console.log("Checking for upload forms to hide");
