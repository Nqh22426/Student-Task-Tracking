                                            ...
                                            <tr data-submission-id="<?php echo $submission['submission_id']; ?>">
                                                ...
                                                <td class="ai-detection-column">
                                                    <?php if ($submission['ai_probability'] !== null): ?>
                                                        <?php 
                                                            $aiPercentage = round($submission['ai_probability'], 1);
                                                            $aiClass = '';
                                                            if ($aiPercentage < 30) {
                                                                $aiClass = 'ai-low';
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('checkAiBtn').addEventListener('click', function() {
            ...
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'check_ai_batch.php', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                ...
                if (xhr.readyState === 4) {
                    ...
                    if (xhr.status === 200) {
                        try {
                            ...
                            if (response.success) {
                                ...
                                response.results.forEach(result => {
                                    ...
                                    if (tr) {
                                        ...
                                        if (result.error) {
                                            aiCell.innerHTML = `<span class="ai-error" title="${result.error}">${result.error}</span>`;
                                        } else if (result.ai_percentage !== null) {
                                            const percentage = result.ai_percentage;
                                            let aiClass = '';
                                            if (percentage < 30) {
                                                aiClass = 'ai-low';
                            ...
                            alert('Error processing AI detection results');
                            // Reset all loading cells back to "Not checked"
                            aiCells.forEach(cell => {
                                if (cell.innerHTML.includes('Checking...')) {
                                    cell.innerHTML = '<span class="text-muted">Not checked</span>';
                        ...
                        alert('Server error during AI detection');
                        // Reset all loading cells back to "Not checked"
                        aiCells.forEach(cell => {
                            if (cell.innerHTML.includes('Checking...')) {
                                cell.innerHTML = '<span class="text-muted">Not checked</span>';
