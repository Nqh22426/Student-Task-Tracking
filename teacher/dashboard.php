                        ...
                        <div class="col">
                            <div class="card class-card">
                                ...
                                <div class="card-body">
                                    ...
                                    <?php
                                    // Count students enrolled in this class - with error handling
                                    try {
                                        $stmt = $pdo->prepare("SELECT COUNT(*) FROM class_enrollments WHERE class_id = ?");
                                        $stmt->execute([$class['id']]);
                                        $student_count = $stmt->fetchColumn();
                                    } catch (PDOException $e) {
                                        $student_count = 0; // Set to 0 if table doesn't exist yet
                                    }
                                    
                                    // Count tasks in this class - with error handling
                                    try {
                                        $stmt = $pdo->prepare("SELECT COUNT(*) FROM tasks WHERE class_id = ?");
                                        $stmt->execute([$class['id']]);
                                        $task_count = $stmt->fetchColumn();
                                    } catch (PDOException $e) {
                                        $task_count = 0; // Set to 0 if table doesn't exist yet
                                    }
                                    ?>
